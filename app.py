...
with st.sidebar:
    ...
    source_option = st.radio("Choose data source", 
                            ...
                            index=3)  # Default to pre-loaded GitHub samples
    ...
    if source_option == "Demo samples":
        ...
        if "gh_files" in st.session_state and st.session_state["gh_files"]:
            # Create mapping from catalog keys to GitHub files
            gh_file_map = {}
                    ...
                    if file_info.get("download_url"):
                        with st.spinner("Downloading..."):
                            ...
                            if content:
                                ...
                                selected_key = selected_gh_key  # Sync with catalog
            ...
            with st.spinner("🔍 Fetching sample files from GitHub..."):
                ...
                if files:
                    st.session_state["gh_files_preloaded"] = {
                        f["name"].upper(): f for f in files if f["type"] == "file"
                    }
                    st.success(f"✅ Found {len(files)} compatible files")
                else:
...
if "jump_to" in st.session_state and st.session_state["jump_to"] != selected_key:
    ...
    # Re-fetch data if using GitHub pre-loaded source
...
st.markdown("---")
st.caption("XRD Rietveld App • Co-Cr Dental Alloy Analysis • Supports .asc, .ASC & .xrdml • GitHub: Maryamslm/XRD-3Dprinted-Ret/SAMPLES") import AutoMinorLocator
import io, os, math, sys, base64, re, xml.etree.ElementTree as ET
from scipy import signal
from scipy.optimize import least_squares
import requests
